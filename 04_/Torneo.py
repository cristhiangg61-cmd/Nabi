...
FS     = 30     # fontsize fijo (BeaNabi) — todas las cajas del bracket
...
def dibujar_panel_inferior(ax, st):
    ...
    # =================================================================
    X3  = 6.0
    BW3 = BOX_W   # referencia dinámica (Flujo_de_Trabajo)
    ...
    # =================================================================
    XF   = 18.0
    BW_F = BOX_W   # referencia dinámica (Flujo_de_Trabajo)
    ...
    draw_box(ax, xaf, Y_CONT, fin[0], "azul" if fin[0] else "empty",
             w=BW_F, h=BOX_H, fontsize=FS)
    draw_box(ax, xbf, Y_CONT, fin[1], "rojo" if fin[1] else "empty",
             w=BW_F, h=BOX_H, fontsize=FS)

    # champ_top proporcional con BOX_H*1.35 (Flujo_de_Trabajo)
    ...
    if st["wb"][4][0]:
        ax.text(XF, Y_LBL + 0.05, "CAMPEON!", ha="center", va="center",
                fontsize=FS, fontweight="bold", color="#ffd700")
        ax.text(XF, Y_LBL - 0.65, "★  ★  ★", ha="center", va="center",
    ...
    for pos, lbl, nombre, estilo, color_lbl in podio:
        ...
        draw_box(ax, 32.0, yp, nombre, estilo if nombre else "empty",
                 w=BOX_W, h=BOX_H, fontsize=FS, z=3)
...
def render(st, banner=None, ruta=None, dpi=130):
    ...
    if banner:
        fig.text(
            0.5, 0.935, banner,
            ha="center", va="top",
            fontsize=25, fontweight="bold", color="#ffe08a",
