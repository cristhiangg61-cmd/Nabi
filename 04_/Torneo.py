...
def dibujar_panel_inferior(ax, st):
    ...
    # =================================================================
    # Mismo ancho y fuente que 3er puesto; alto ajustado para que 4 filas quepan
    GAP_P   = 0.30
    PH      = (Y_HEAD - 1.10 - 0.3 - 3 * GAP_P) / 4   # ~1.90
    PH1     = PH / 2 - 0.04
    ROW_P   = PH + GAP_P
    Y_FIRST = Y_HEAD - 1.10
    ...
    X_MED   = 20.5
    X_LBL   = 21.7
    X_BOX   = 36.0 - BOT_W / 2 - 0.3

    ax.text((X_MED + X_BOX) / 2, Y_HEAD, "PODIO", ha="center", va="center",
    ...
    for idx_p, (pos, lbl, equipo, estilo, color_lbl) in enumerate(podio):
        ...
        if pos <= 3:
            draw_medal(ax, X_MED, yp, pos, r=0.42)
        else:
        ...
        draw_box_duo(ax, X_BOX, yp, equipo,
                     ...
                     w=BOT_W, h=PH, h1=PH1, fontsize=FS, z=3)
