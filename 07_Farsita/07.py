...
def dibujar_bracket(ax, st):
    ...
    for lado in ("izq", "der"):
        ...
        # Conectores bracket rondas 1-2 (la ronda 3 se dibuja en el bloque central)
        for r in range(1, 3):
            n_match = N_SIDE >> r
    ...
    # ── CENTRO: finalistas visibles a los lados + caja campeón en el medio ──
    y_fin = _get_y(3, 0, oct_ys)
    cx    = 0.0
    # Finalistas se dibujan más separados para no solaparse con la caja campeón
    X_FIN_IZQ = -3.2
    X_FIN_DER = +3.2

    # Redibujar finalistas en posición más separada
    ...
    # Conectores semis → finalistas (ronda 2 → posiciones separadas)
    oct_ys_local = _oct_ys()
    y_semi_izq = _get_y(2, 0, oct_ys_local)
    y_semi_der = _get_y(2, 0, oct_ys_local)  # misma Y (ronda 2 slot 0 ambos lados)
    # izquierdo: borde der de semi izq (x=XS_IZQ[2]) → borde izq de finalista izq
    draw_line(ax, XS_IZQ[2] + BOX_W / 2, y_fin, X_FIN_IZQ - BOX_W / 2, y_fin, lw=1.5)
    # derecho: borde izq de semi der (x=XS_DER[2]) → borde der de finalista der
    draw_line(ax, XS_DER[2] - BOX_W / 2, y_fin, X_FIN_DER + BOX_W / 2, y_fin, lw=1.5)
    # Conectores: finalistas → caja campeón
...
def dibujar_panel_inferior(ax, st):
    ...
    # champ_top usa BOT_H escalado para el conector
    champ_top = Y_WIN + (BOT_H * 1.35) / 2
    draw_match_connector_horiz(ax, xaf, xbf, Y_CONT, XF, champ_top)

    # Caja campeón también usa BOT_W/BOT_H escalados
    draw_box_champ(ax, XF, Y_WIN, st["wb"][4][0],
                   w=BOT_W * 1.35, h=BOT_H * 1.35, fontsize=FS + 2)

    if st["wb"][4][0]:
        champ_bot = Y_WIN - (BOT_H * 1.35) / 2
        ax.text(XF, champ_bot - 0.25, "CAMPEON!", ha="center", va="top",
                fontsize=FS, fontweight="bold", color="#ffd700")
        ax.text(XF, champ_bot - 0.90, "★  ★  ★", ha="center", va="top",
