...
def dibujar_bracket(ax, st):
    ...
    for lado in ("izq", "der"):
        ...
        # Cajas (todas con BOX_W, BOX_H, FS constantes)
        for r in range(4):
            n_slots = N_SIDE >> r
    ...
    # ── CENTRO: etiqueta GRAN FINAL + caja campeón + CAMPEON! + estrellas ──
    y_fin  = _get_y(3, 0, oct_ys)   # Y del finalista izquierdo (== derecho)
    cx     = 0.0                     # centro horizontal de la figura

    # Etiqueta "★ GRAN FINAL ★" justo encima de la caja campeón
    ...
    # Caja del campeón en el centro del bracket (más angosta para no tapar finalistas)
    draw_box_champ(ax, cx, y_fin, st["wb"][4][0],
                   w=BOX_W * 1.05, h=BOX_H * 1.35, fontsize=FS + 2)

    # Líneas conectoras de los finalistas hacia la caja central
    # Izquierdo: desde borde derecho de x=-1.5 hasta borde izq de cx
    x_fin_izq = XS_IZQ[3] + BOX_W / 2
    x_fin_der = XS_DER[3] - BOX_W / 2
    champ_izq = cx - (BOX_W * 1.35) / 2
    champ_der = cx + (BOX_W * 1.35) / 2
    draw_line(ax, x_fin_izq, y_fin, champ_izq, y_fin, lw=1.5)
    draw_line(ax, x_fin_der, y_fin, champ_der, y_fin, lw=1.5)

    # Texto "CAMPEON!" y estrellas debajo si ya hay campeón
    if st["wb"][4][0]:
