...
def dibujar_panel_inferior(ax, st):
    ...
    # Se escala solo por la diferencia de rango X/Y entre paneles, SIN
    # multiplicar por la razón de alturas del gridspec (_RATIO_TOP/_RATIO_BOT),
    # que inflaba artificialmente BOT_H y hacía los bloques demasiado grandes.
    ...
    BOT_W  = BOX_W * (_X_RANGE_BOT / _X_RANGE_TOP)          # ≈ 3.49
    BOT_H  = BOX_H * (_Y_RANGE_BOT / _Y_RANGE_TOP)          # ≈ 1.14  (sin inflar)
    BOT_H1 = BOT_H / 2 - 0.04                               # alto de cada sub-fila
    ...
    # =================================================================
    #  SECCIÓN 2 — PODIO  (lado derecho, xlim 18..36, centro = 27)
    # =================================================================
    # Geometría del podio centrada alrededor de x=27.
    # Estructura de cada fila: [medalla] [etiqueta] [bloque equipo]
    # distribuidos simétricamente alrededor del centro de la sección.
    POD_CX  = 27.0                  # centro horizontal de la sección podio
    MED_R   = 0.38                  # radio de medalla (más compacto)
    GAP_P   = 0.28                  # espacio vertical entre filas
    ROW_P   = BOT_H + GAP_P        # paso de fila (proporcional a BOT_H real)

    # Las 4 filas deben caber entre Y_HEAD-0.8 y ~1.0
    # Calculamos Y_FIRST para que el conjunto quede centrado verticalmente
    total_height = 3 * ROW_P + BOT_H   # span completo de las 4 filas
    Y_FIRST = Y_HEAD - 0.95            # primera fila (Oro), debajo del título

    # Posiciones X: medalla a la izquierda, etiqueta junto a ella, caja a la derecha
    X_MED  = POD_CX - BOT_W / 2 - 1.50   # medalla: ~1.5 u antes del bloque
    X_LBL  = X_MED  + MED_R + 0.55        # etiqueta: pegada a la derecha de la medalla
    X_BOX  = POD_CX + 0.80                # bloque: centrado a la derecha del título

    ax.text(POD_CX, Y_HEAD, "PODIO", ha="center", va="center",
            fontsize=FS, fontweight="bold", color="#ffffff")
    ax.plot([19.0, 36.0], [Y_HEAD - 0.52, Y_HEAD - 0.52],
    ...
    for idx_p, (pos, lbl, equipo, estilo, color_lbl) in enumerate(podio):
        yp = Y_FIRST - idx_p * ROW_P

        if pos <= 3:
            draw_medal(ax, X_MED, yp, pos, r=MED_R)
        else:
        ...
        draw_box_duo(ax, X_BOX, yp, equipo,
