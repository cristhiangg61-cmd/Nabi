...
def dibujar_bracket(ax, st):
    ...
    # ── CENTRO: finalistas en columna SEMIFINAL + caja campeón en el centro ──
    # En el PDF: los finalistas están en x=±5.5 (columna semi) y la caja campeón
    # queda sola en el centro, con líneas horizontales directas a ella.
    y_fin = _get_y(3, 0, oct_ys)
    cx    = 0.0

    # Los finalistas van en la misma X que las semis (±5.5) pero en la Y de la final
    X_FIN_IZQ = XS_IZQ[2]   # -5.5
    X_FIN_DER = XS_DER[2]   # +5.5
    ...
    # Conectores: semis → finalistas (desde y_semi hacia y_fin, misma X)
    y_semi_izq = _get_y(2, 0, oct_ys)
    y_semi_der = _get_y(2, 0, oct_ys)
    # Línea vertical de la semi izq hacia abajo hasta y_fin, luego horizontal
    draw_line(ax, XS_IZQ[2] + BOX_W / 2, y_semi_izq,
              XS_IZQ[2] + BOX_W / 2, y_fin, lw=1.5)
    draw_line(ax, XS_DER[2] - BOX_W / 2, y_semi_der,
              XS_DER[2] - BOX_W / 2, y_fin, lw=1.5)
    # Conectores directos: finalistas → caja campeón (línea horizontal)
